...
class CLCarsTrucks(Spider):
    ...
    def start_requests(self):
        ...
        yield Request(self.proto + self.city + '.' + self.domain + '/search/cto?' + urlencode(self.getSearchArgs()))
    ...
    def parse_detail_page(self, response):
        ...
        try:
            ...
            item['price'] = ""
        item['info'] = response.xpath('//*[@id="postingbody"]').extract()[0]
        ...
        item['post_id_number'] = response.xpath('//*[@class = "postinginfo"]/text()').extract_first()
        item['URLcontact_info'] = self.proto + self.city + '.' + self.domain + response.css("a.showcontact ::attr(href)").extract_first()
    ...
    def processATTRS(self, attrs, item):
        ...
        allowed = ['odometer', 'VIN', 'title status', 'transmission', 'auto_descr_short', 'fuel']
        attr_found = {}
        for a in attrs:
